 ...
 *
 * The angle the earth has turned under the fixed stars, which is what converts
 * between a right ascension in the sky and a longitude on the ground. Two
 * things ask: the sun below, whose position is computed in the sky and drawn on
 * the map, and the sky behind the globe, which is the same conversion run the
 * other way.
 */
export function gmst(date = new Date()) {
  const n = (date.getTime() - J2000) / 86400000;
  return (18.697374558 + 24.06570982441908 * n) % 24;
}
...
export function subsolar(date = new Date()) {
  ...
  const lon = rightAscension - gmst(date) * 15;
