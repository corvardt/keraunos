...
// to serve — the PNG comes back 200 — it is a refusal to be read back, and the
// IR layer reads every tile back. `flat()` asks whether a tile is a single
...
// answers. So the request has to be made by something that is not a browser tab,
// and this is the smallest such thing that already exists on the way to one.
//
...
//
// The query is the WMS call `ir.js` already built, forwarded as-is. Nothing here
// knows what a tile is or when a scan was taken; the map decides that, the same
// way it decides it for the dishes it can reach itself.
...
// narrow enough to write down — and a request that does not match is a bug or
// somebody else's traffic, and both want the same answer.
//
...
export async function onRequestGet({ request }) {
  ...
  const answer = await fetch(`${UPSTREAM}?${out}`, {
    ...
    // answers the occasional 500 to a request that succeeds a second later —
